...
run-nginx:
	docker run -i -d -p 80:80 -v ~/opt:/opt -t saint/nginx

in-nginx:
	docker run -i -p 80:80 -v ~/opt:/opt -t saint/nginx /bin/bash

build-php:
	docker build -t saint/php ./php

run-php:
	docker run -i -d -p 9000:9000 -v ~/opt:/opt -t saint/php

in-php:
	docker run -i -p 9000:9000 -v ~/opt:/opt -t saint/php /bin/bash

build-mysql:
	docker build -t saint/mysql ./mysql

run-mysql:
	docker run -i -d -p 3306:3306 -v ~/opt/data/mysql:/var/lib/mysql -e MYSQL_ROOT_PASSWORD=123456 -t saint/mysql

in-mysql:
	docker run -i -p 3306:3306  -v ~/data/mysql:/var/lib/mysql -e MYSQL_ROOT_PASSWORD=123456 -t saint/mysql /bin/bash

build-node:
	docker build -t saint/node ./node

run-node:
	docker run -i -d -p 8001:8001 -v ~/opt:/opt -t saint/node

in-node:
	docker run -i -p 8001:8001 -v ~/opt:/opt -t saint/node /bin/bash

build-elasticsearch:
	docker build -t saint/elasticsearch ./elasticsearch

run-elasticsearch:
	docker run -i -d -p 9200:9200 -p 9300:9300 -v ~/opt/data/elasticsearch:/usr/share/elasticsearch/data -t saint/elasticsearch

in-elasticsearch:
	docker run -i -p 9200:9200 -p 9300:9300 -v ~/opt/data/elasticsearch:/usr/share/elasticsearch/data -t saint/elasticsearch /bin/bash

build-gearman:
	docker build -t saint/gearman ./gearman

run-gearman:
	docker run -d -p 4730:4730 -v ~/opt:/opt -it saint/gearman
